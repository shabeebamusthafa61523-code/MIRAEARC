...
export default function App() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setProgress((prev) => (prev >= 94 ? 78 : prev + 1));
    }, 3000);
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col justify-between architectural-grid-light bg-slate-50 text-slate-900 font-sans selection:bg-orange-500 selection:text-white relative">
      ...
      <main className="w-full max-w-4xl mx-auto px-6 z-10 flex flex-col items-center text-center my-auto py-1">
        ...
        {/* Animated Architectural Loader & Status */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 w-full max-w-2xl mb-4">
          ...
          {/* Email Card (Direct Clickable Mailto Anchor + Copy Button) */}
          <a
            href={`mailto:${emailAddress}`}
            onClick={(e) => {
              // Standard mailto link handler
            }}
            className="group relative bg-white/90 hover:bg-white p-3.5 sm:p-4 rounded-xl flex flex-col justify-between text-left cursor-pointer border border-slate-200 shadow-xs hover:shadow-md hover:border-orange-400 transition-all block"
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-1 rounded-md bg-slate-100 hover:bg-orange-500 text-slate-600 hover:text-white transition-colors cursor-pointer"
                >
                  <Copy className="w-3.5 h-3.5" />
                </button>
                <span className="p-1 rounded-md bg-slate-100 text-slate-400 group-hover:text-orange-600 transition-colors">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 mt-0.5 flex items-center gap-1">
                Click to send email directly <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform text-orange-500" />
              </p>
            </div>
          </a>
          ...
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <span className="p-1 rounded-md bg-slate-100 text-slate-400 group-hover:text-rose-600 transition-colors">
                <ExternalLink className="w-3.5 h-3.5" />
              </span>
            </div>
            <div>
